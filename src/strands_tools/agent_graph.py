...
def create_rich_status_panel(console: Console, status: Dict) -> str:
    ...
    for node in status["nodes"]:
        node_info = [
            ...
            f"  [bold green]Model:[/bold green] {node['model_id']}",
...
class AgentNode:
    def __init__(self, node_id: str, role: str, model_id: str, system_prompt: str):
        self.id = node_id
        self.role = role
...
class AgentGraph:
    ...
    def add_node(self, node_id: str, role: str, model_id: str, system_prompt: str):
        with self.lock:
            node = AgentNode(node_id, role, model_id, system_prompt)
