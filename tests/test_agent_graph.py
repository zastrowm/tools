...
def test_create_rich_status_panel(mock_console):
    ...
    status = {
        ...
        "topology": "star",
        "nodes": [
            {"id": "node1", "role": "test_role", "model_id": "test-model", "neighbors": ["node2"], "queue_size": 0}
        ],
    }
...
class TestAgentNode:
    ...
    def test_init(self):
        ...
        node = AgentNode("test_node", "test_role", "test_model", "Test system prompt")
        assert node.id == "test_node"
        assert node.role == "test_role"
        assert node.model_id == "test_model"
    ...
    def test_add_neighbor(self):
        ...
        node1 = AgentNode("node1", "role1", "model1", "System prompt 1")
        node2 = AgentNode("node2", "role2", "model2", "System prompt 2")
...
class TestAgentGraph:
    ...
    def test_add_node(self, tool_context):
        ...
        graph = AgentGraph("test_graph", "star", tool_context)
        node = graph.add_node("test_node", "test_role", "test_model", "Test system prompt")
    ...
    def test_add_edge(self, tool_context):
        ...
        graph = AgentGraph("test_graph", "star", tool_context)
        node1 = graph.add_node("node1", "role1", "model1", "prompt1")
        node2 = graph.add_node("node2", "role2", "model2", "prompt2")
    ...
    def test_start(self, tool_context, mock_thread_pool):
        ...
        graph.add_node("node1", "role1", "model1", "prompt1")
        graph.start()
    ...
    def test_get_status(self, tool_context):
        ...
        graph.add_node("node1", "role1", "model1", "prompt1")
        graph.add_node("node2", "role2", "model2", "prompt2")
        graph.add_edge("node1", "node2")
        ...
        tool_use = {
            ...
            "input": {
                ...
                "topology": {
                    "type": "star",
                    "nodes": [
                        {"id": "central", "role": "coordinator", "model_id": "test-model", "system_prompt": "test"}
                    ],
                },
            },
        }
